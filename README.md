...
可选 `notify.webhook_url`（默认空=关闭）：有**新** IP 加白或 push 失败时，异步 POST 一条 JSON 事件（`{"event","time","detail"}`）到该 URL，可指向 Telegram Bot API 或任意 IM 的 incoming webhook。发送失败只记审计日志，不影响认证和 push；刷新 TTL 不通知。URL 可能内含 bot token，因此绝不写入日志；非回环地址必须用 https。
