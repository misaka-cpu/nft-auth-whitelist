...
可选 `notify.webhook_url`（默认空=关闭）：有**新** IP 加白或 push 失败时，异步 POST 一条 JSON 事件（`{"event","time","detail"}`）到该 URL。发 Telegram 就再配 `notify.telegram_chat_id`，payload 自动变为 Bot API 的 `{"chat_id","text"}`，`webhook_url` 填 `https://api.telegram.org/bot<TOKEN>/sendMessage`。发送失败只记审计日志，不影响认证和 push；刷新 TTL 不通知。URL 可能内含 bot token，因此绝不写入日志；非回环地址必须用 https。
...
- 应急冻结：怀疑凭据泄露时 `touch` 配置的 `freeze_file`（默认 `<data_dir>/freeze`），认证页立即对所有人返回 503、不再写入白名单；`rm` 恢复，均无需重启。已有白名单条目不受影响。

Cloudflare Access 前门 + trusted proxy 的**具体配置**（DNS、Access policy、Caddy/Nginx 传 `CF-Connecting-IP`、`server.json` 示例）见 [docs/public-deployment.md](./docs/public-deployment.md) 的「Cloudflare Access 设置」一节；安全边界另见 [SECURITY.md](./SECURITY.md)。
