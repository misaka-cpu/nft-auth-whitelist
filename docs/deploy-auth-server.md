...
- 安装 `/etc/systemd/system/nft-auth-server.service` 并 `daemon-reload`（**不 enable/start**）。
...
```bash
sudo systemctl enable --now nft-auth-server.service
...
sudo systemctl restart nft-auth-server.service
```
