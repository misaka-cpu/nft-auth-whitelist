...
Use these when staging a new RFC internal machine or po0 host for the first time. For the detailed real-host SSH push sequence, follow [Real-host SSH Push Checklist](./real-host-ssh-push-checklist.md).

- [ ] Install the RFC internal machine auth-server role with `sudo ./install.sh --role auth-server`.
...
- [ ] Run the RFC internal machine auth-server manually or via a reviewed systemd unit.
- [ ] Perform the RFC internal machine auth-server browser login and confirm the page records the expected client IP/CIDR.
...
- [ ] Configure the RFC internal machine `po0-shadow` push target with strict host key checking and a pinned `known_hosts_file`.
- [ ] Run `bash scripts/preflight-push-target.sh --target po0-shadow --ssh-test` on the RFC internal machine.
...
Repeat the RFC internal machine auth-server browser login validation after changing any of these:
